/**
  ******************************************************************************
  * @file	: 11_SldrDALtchMPBttn_2b.cpp
  ...
  * the on-board led. The isOn attribute flag status enables and disables the pin output,
  * while the curOtpVal attribute will set the PWM parameters of that same pin.
  *
  * The second level action output connected to a led shows when the the secondary facilities are
  * working.
  *
...
void mainCtrlTsk(void *pvParameters)
{
	...
	tstBttn.setScndModActvDly(1500);
	tstBttn.setSldrDirDn();
	tstBttn.setSwpDirOnPrss(true);
	tstBttn.setSwpDirOnEnd(false);
