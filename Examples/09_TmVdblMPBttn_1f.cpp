/**
  ...
  * 	- A LED attached to GPIO_C01 to visualize the FnWhnTrnOnVdd() and FnWhnTrnOffVdd() activity
  *
  * ### This example creates three Tasks, a timer, and four dedicated functions:
  *
  ...
  *
  * - The third functions is to be executed when the MPB enters the "isVoided state", please refer to the
  * **setFnWhnTrnOnVddPtr()** method for details.
  *
  * - The fourth functions is to be executed when the MPB exits the "isVoided state", please refer to the
  * **setFnWhnTrnOffVddPtr()** method for details.
  *
  * 	@author	: Gabriel D. Goldman
  *
  * 	@date	: 	01/01/2024 First release
  * 				02/09/2024 Last update
  *
  ******************************************************************************
...
gpioPinId_t ledFnTrnOnOffVdd{GPIOC, GPIO_PIN_1};	// Pin 0b 0100 0000 0000 0000
...
void mainCtrlTsk(void *pvParameters)
{
	...
	tstBttn.setFnWhnTrnOffVddPtr(fnExecTrnOffVdd);
...
void fnExecTrnOnVdd(){
	HAL_GPIO_WritePin(ledFnTrnOnOffVdd.portId, ledFnTrnOnOffVdd.pinNum, GPIO_PIN_SET);

	return;
}

void fnExecTrnOffVdd(){
	HAL_GPIO_WritePin(ledFnTrnOnOffVdd.portId, ledFnTrnOnOffVdd.pinNum, GPIO_PIN_RESET);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(ledFnTrnOnOffVdd.portId, ledFnTrnOnOffVdd.pinNum, GPIO_PIN_RESET);
  /*Configure GPIO pin : ledFnTrnOnOff*/
  GPIO_InitStruct.Pin = ledFnTrnOnOffVdd.pinNum;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(ledFnTrnOnOffVdd.portId, &GPIO_InitStruct);
}
