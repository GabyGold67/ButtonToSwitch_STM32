/**
  ******************************************************************************
  * @file	: 12_DDlydDALtchMPBttn_1e.cpp
  ...
  *
  * ### This example creates four Tasks, a timer, and four dedicated functions:
  *
  * - The first task instantiates the DDlydDALtchMPBttn object in it and checks it's
  ...
  *
  * - The fourth task is analog to the third, but for being executed while the MPB is in "isOn Secondary state"
  * Similar cautions must be taken.
  *
  ...
  *
  * - The third functions is to be executed when the MPB enters the "isOnScndry state", please refer to the
  * **setFnWhnTrnOnPtr()** method for details.
  *
  * - The fourth functions is to be executed when the MPB enters the "isOff state", please refer to the
  * **setFnWhnTrnOffPtr()** method for details.
  *
...
gpioPinId_t ledTskWhlOnScndry{GPIOA, GPIO_PIN_10};	// Pin 0b 0000 0100 0000 0000
gpioPinId_t ledFnTrnOnOff{GPIOB, GPIO_PIN_14};	// Pin 0b 0100 0000 0000 0000
gpioPinId_t ledFnTrnOnOffScndry{GPIOA, GPIO_PIN_4};	// Pin 0b 0100 0000 0000 0000

TaskHandle_t mainCtrlTskHndl {NULL};
TaskHandle_t dmpsOutputTskHdl;
TaskHandle_t dmpsActWhlOnTskHndl;
TaskHandle_t dmpsActWhlOnScndryTskHndl;
...
void dmpsActWhlOnScndryTsk(void *pvParameters);
// SW Timers Callbacks
...
void fnExecTrnOn();
void fnExecTrnOff();
void fnExecTrnOnScndry();
void fnExecTrnOffScndry();
/* USER CODE END FP */
...
int main(void)
{
  ...
  xReturned = xTaskCreate(
		  dmpsActWhlOnScndryTsk,
		  "ExecWhileOnSecTask",
		  256,
		  NULL,
		  configTIMER_TASK_PRIORITY,
		  &dmpsActWhlOnScndryTskHndl
		  );
  if(xReturned != pdPASS)
	  Error_Handler();
...
/* USER CODE BEGIN BEFORE STARTING SCHEDULER: SETUPS, OBJECTS CREATION, ETC. */
	...
	vTaskSuspend(dmpsActWhlOnScndryTskHndl);
...
void mainCtrlTsk(void *pvParameters)
{
	...
	tstBttn.setTaskWhileOnScndry(dmpsActWhlOnScndryTskHndl);
	tstBttn.setFnWhnTrnOnPtr(&fnExecTrnOn);
	tstBttn.setFnWhnTrnOffPtr(&fnExecTrnOff);
	tstBttn.setFnWhnTrnOnScndryPtr(&fnExecTrnOnScndry);
	tstBttn.setFnWhnTrnOffScndryPtr(&fnExecTrnOffScndry);
	tstBttn.begin(10);
...
void dmpsActWhlOnScndryTsk(void *pvParameters){
	const unsigned long int swapTimeMs{125};
	unsigned long int strtTime{0};
	unsigned long int curTime{0};
	unsigned long int elapTime{0};
	bool blinkOn {false};
	...
	for(;;){
		...
		if (elapTime > swapTimeMs){
			blinkOn = !blinkOn;
			if(blinkOn)
				HAL_GPIO_WritePin(ledTskWhlOnScndry.portId, ledTskWhlOnScndry.pinNum, GPIO_PIN_SET);
			else
				HAL_GPIO_WritePin(ledTskWhlOnScndry.portId, ledTskWhlOnScndry.pinNum, GPIO_PIN_RESET);
			strtTime = curTime;
		}
	}
...
void fnExecTrnOnScndry(){
	HAL_GPIO_WritePin(ledFnTrnOnOffScndry.portId, ledFnTrnOnOffScndry.pinNum, GPIO_PIN_SET);

	return;
}

void fnExecTrnOffScndry(){
	HAL_GPIO_WritePin(ledFnTrnOnOffScndry.portId, ledFnTrnOnOffScndry.pinNum, GPIO_PIN_RESET);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(ledIsOnScndry.portId, ledIsOnScndry.pinNum, GPIO_PIN_RESET);
  /*Configure GPIO pin : ledIsOnScndry_Pin */
  ...
  /*Configure GPIO pin Output Level for ledTskWhlOnScndry*/
  HAL_GPIO_WritePin(ledTskWhlOnScndry.portId, ledTskWhlOnScndry.pinNum, GPIO_PIN_RESET);
  /*Configure GPIO pin : ledIsEnabled_Pin */
  GPIO_InitStruct.Pin = ledTskWhlOnScndry.pinNum;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(ledTskWhlOnScndry.portId, &GPIO_InitStruct);
  ...
  HAL_GPIO_WritePin(ledFnTrnOnOffScndry.portId, ledFnTrnOnOffScndry.pinNum, GPIO_PIN_RESET);
  /*Configure GPIO pin : ledFnTrnOnOff*/
  GPIO_InitStruct.Pin = ledFnTrnOnOffScndry.pinNum;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(ledFnTrnOnOffScndry.portId, &GPIO_InitStruct);
}
